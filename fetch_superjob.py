...
def predict_rub_salary_sj(superjob_secret_key, vacancy, area):
    ...
    for salary in fetch_all_sj_salaries(superjob_secret_key, vacancy, area):
        if salary and salary[2] == "rub" and (salary[1]+salary[0]) != 0:
            average_salaries.append(predict_salary(int(salary[0]),
                                                   int(salary[1])))
...
def fetch_sj_vacancies_pages_amount(superjob_secret_key, search_query, area):
    amount_of_vacancies = fetch_sj_vacancies_amount(superjob_secret_key,
                                                    search_query, area)
    return (amount_of_vacancies//20)+1


def fetch_sj_vacancies_amount(superjob_secret_key, search_query, area):
    vacancies = fetch_sj_vacancies(superjob_secret_key, keyword=search_query,
                                   town=area)
...
def fetch_all_sj_salaries(superjob_secret_key, search_query, area):
    pages_found = fetch_sj_vacancies_pages_amount(superjob_secret_key,
                                                  search_query, area)
    all_sj_salaries = []
    for page in range(pages_found):
        vacancies = fetch_sj_vacancies(superjob_secret_key,
        ...
        all_sj_salaries += zip(all_sj_salaries_from, all_sj_salaries_to,
